...
namespace MaslovaT_task3_practice2024
{
    ...
    {
        /// <summary>
        ///  Даны координаты нахождения коня на шахматной доске. Требуется определить, бьет ли конь фигуру, 
        ///  стоящую на другой указанной клетке за один ход. Координаты коня и фигуры вводятся согласно шахматной
        ///  записи через пробел в одну строку. В качестве выходного сообщения выводится одна из строк: "Конь сможет
        ///  побить фигуру", "Конь не сможет побить фигуру", "Введены некорректные координаты"
        /// </summary>
        static void Main()
        {
            ...
            string mainPiecePosition = inputLine.Split()[0];
            string otherPiecePosition = inputLine.Split()[1];

            if (!TwoChessPiecesInputValid(mainPiecePosition, otherPiecePosition))
            {
                Console.WriteLine("Вы ввели некорректные координаты");
                Console.ReadKey();
                return;
            ...
            if (KnightCanEatPiece(mainPiecePosition, otherPiecePosition))
            {
